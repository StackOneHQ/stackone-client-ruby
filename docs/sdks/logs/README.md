...
* [get_action_policy_log](#get_action_policy_log) - Get Action Policy Log
...
* [get_platform_log_dimensions](#get_platform_log_dimensions) - Get Platform Log Dimensions
...
## get_action_policy_log

Returns the runtime policy decision recorded for an action run: the rules that decided, the version each was at when it decided, the level that decided, the acting principal, and anything a monitored rule observed. Available when the request was evaluated by the policy layer, which the log row reports through `policy_result`.

### Example Usage

<!-- UsageSnippet language="ruby" operationID="stackone_get_action_policy_log" method="get" path="/logs/actions/{actionRunId}/policy" -->
```ruby
require 'stackone_client'
...
res = s.logs.get_action_policy_log(action_run_id: '<id>')

unless res.policy_log.nil?
  # handle response
end
...
**[T.nilable(Models::Operations::StackoneGetActionPolicyLogResponse)](../../models/operations/stackonegetactionpolicylogresponse.md)**
...
## get_platform_log_dimensions

Returns the distinct values available for the requested platform-log dimensions (for example event type, actor, or source), with a count of matching events for each. Use to populate the filter controls on the platform trail without first reading a page of it. Pass `filters` to narrow the values to the rows a listing with those same filters would return, so the counts describe what the caller is looking at. Requires access to every account in the project, for the same reason listing the trail does.

### Example Usage

<!-- UsageSnippet language="ruby" operationID="stackone_get_platform_log_dimensions" method="post" path="/logs/platform/dimensions" -->
```ruby
require 'stackone_client'
...
req = Models::Shared::PlatformDimensionsPostDto.new(
  dimensions: [],
  filters: Models::Shared::PlatformDimensionsPostDtoFilters.new(
    action: [
      'create',
      'delete',
    ],
...
res = s.logs.get_platform_log_dimensions(request: req)
...
| Parameter                                                                                     | Type                                                                                          | Required                                                                                      | Description                                                                                   |
| --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `request`                                                                                     | [Models::Shared::PlatformDimensionsPostDto](../../models/shared/platformdimensionspostdto.md) | :heavy_check_mark:                                                                            | The request object to use for the request.                                                    |

### Response

**[T.nilable(Models::Operations::StackoneGetPlatformLogDimensionsResponse)](../../models/operations/stackonegetplatformlogdimensionsresponse.md)**
