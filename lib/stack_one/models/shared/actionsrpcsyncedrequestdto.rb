  ...
  module Models
    module Shared
      ...
      class ActionsRpcSyncedRequestDto
        ...
        # OpenSearch aggregations object. When set, aggregation results are returned instead of records, and `source`, `sort`, `page_size` and `skip` are ignored (`filter` still applies; `page_size` and `skip` must still be within their limits).
        ...
        # OpenSearch-style filter (Query DSL) (term / terms / range / wildcard / match / match_phrase / exists / bool) over synced fields. A filter is one clause: an object with exactly one query type, keyed by field name, e.g. {"term": {"<field>": <value>}}, {"terms": {"<field>": [...]}}, {"range": {"<field>": {"gte": ...}}}; combine clauses with {"bool": {"filter": [...], "must_not": [...]}}. Bare {"<field>": <value>} maps are rejected. match / match_phrase do word-level search (string fields only, case-insensitive); wildcard matches patterns (case-sensitive). Use field names exactly as returned in the action's "queryable_fields".
